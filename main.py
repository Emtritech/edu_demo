...
@app.get("/")
async def root():
    return {"message": "Welcome to Emtritech Edutech Demo API"}
...
async def signup(user: schemas.Signup):
    try:
        ...
        # # Upload files to Firebase
        user = user.dict()
        user["date_of_birth"] = datetime.strftime(user["date_of_birth"], "%Y-%m-%d")
...
async def register_item(
    ...
    passport_url = crud.upload_to_firebase(passport, "passport")
    ...
    date_str = registered_date_obj.strftime("%Y-%m-%d")

    # Save item registration
    credentials = schemas.Credentials(
        state_of_origin_cert=state_of_origin_cert_url,
        date_of_birth_cert=date_of_birth_cert_url,
        waec=waec_url,
    ...
    if not await crud.save_item_registration(credentials):
